...
export default function SupportPage({initialLocale}:{initialLocale:LandingLocale}){
  ...
  return <main className="support-page">
    <header className="support-header"><Link className="brand" href="/" aria-label="Landeo"><span className="brand-mark" aria-hidden="true">L</span><span>Landeo</span></Link><nav aria-label={text.nav.label}><Link href="/">{text.nav.home}</Link><Link href="/pricing">{text.nav.pricing}</Link><Link href="/login">{text.nav.login}</Link><div className="language-switcher" role="group" aria-label={text.language}><span aria-hidden="true">◎</span><button type="button" className={locale==="es"?"active":""} aria-pressed={locale==="es"} onClick={()=>changeLocale("es")}>ES</button><i/><button type="button" className={locale==="en"?"active":""} aria-pressed={locale==="en"} onClick={()=>changeLocale("en")}>EN</button></div></nav></header>
    ...
    <footer className="support-footer"><div><Link className="brand" href="/"><span className="brand-mark" aria-hidden="true">L</span><span>Landeo</span></Link><p>{text.footer}</p></div><nav><Link href="/pricing">{text.nav.pricing}</Link><Link href="/privacy">{text.privacy}</Link><Link href="/terms">{text.terms}</Link></nav><small>© 2026 Landeo</small></footer>
  </main>;
}
