...
const copy={
  es:{
    ...
    stripe:"Pago seguro con Stripe",stripeBody:"Stripe gestiona el cobro y la suscripción. Landeo activa el plan únicamente después de recibir una confirmación válida.",footer:"Buscar trabajo, sin repetir tu historia.",privacy:"Privacidad",terms:"Términos",affiliate:"Afiliados",
  },
  en:{
    ...
    stripe:"Secure payment with Stripe",stripeBody:"Stripe manages payment and subscriptions. Landeo activates a plan only after receiving a valid confirmation.",footer:"Job hunting, without repeating your story.",privacy:"Privacy",terms:"Terms",affiliate:"Affiliates",
  },
} as const;
...
export default function PricingPage({initialLocale}:{initialLocale:LandingLocale}){
  ...
  return <main className="pricing-page-new">
    ...
    <footer className="pricing-footer"><div><Link className="brand" href="/"><span className="brand-mark">L</span><span>Landeo</span></Link><p>{text.footer}</p></div><nav><Link href="/affiliate-program">{text.affiliate}</Link><Link href="/privacy">{text.privacy}</Link><Link href="/terms">{text.terms}</Link></nav><small>© 2026 Landeo</small></footer>
  </main>;
}
